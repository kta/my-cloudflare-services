...
2. **e2e どうしの日の取り合い**。予約フローは `DAY` を暦で決め打ちしており、
   実時間がその日に追いつくと、実時刻の当日を使う来店受付の e2e と同じ枠を奪い合って
   409 `slot_taken` になる。**決め打ちの日はいつか必ず当日になる**ので、
   ファイルごとに日を分けるだけでは足りず、いずれ当日からの相対で決める必要がある。
