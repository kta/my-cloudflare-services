...
describe('GET /api/staff/ledger', () => {
  ...
  it('1 日分を db.batch() 1 回・16 文以内で読む（バッチの外の文も数える）', async () => {
    ...
    // 台帳 1 画面の初回描画で叩く D1 の文は 15 文 —— バッチの 11 文に、
    // 組織が同期済みかを見る 1 文（`requireActiveOrg`）と店舗の実在を見る 1 文、
    // **帯のお名前と来店回数を読む 1 文**（P4。`04-api.md` §3.6 が数える `customers`）、
    // **「いまお待ち N名」と次の整理番号を読む 1 文**（P5。受付パネルの 3 欄はこの 1 文で
    // まかない、人数と番号で 2 往復しない）が付く。
    // バッチが 11 文なのは、ご用件の短い名前を**別の 1 文**で読むためである
    // （帯 1 本ごとに `visit_purposes` を引き直すと、読む行数が帯の数だけ掛け算になる）。
    // 07-nfr.md の上限は 16 本なので 1 本の余裕がある。増やすときはここを直す。
