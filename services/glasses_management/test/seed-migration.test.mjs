...
test('ローカル seed は旧組織の全スコープ行を eye へ移す SQL を出す', () => {
  const statements = legacySeedMigrationStatements(false)

  assert.ok(statements[0].includes("DELETE FROM organizations WHERE id = 'eye'"))
  ...
  assert.equal(
    statements.at(-1),
    "UPDATE organizations SET id = 'eye', name = 'EYE' WHERE id = 'org-eye-seed' AND NOT EXISTS (SELECT 1 FROM organizations WHERE id = 'eye');",
  )
})
...
test('旧 seed を入れたローカル DB を eye へ移行する', () => {
  ...
  const schema = [
    ...
    "INSERT INTO organizations VALUES ('org-eye-seed', 'EYE');",
    "INSERT INTO organizations VALUES ('eye', 'eye');",
    "INSERT INTO stores VALUES ('org-eye-seed');",
    "INSERT INTO staff VALUES ('org-eye-seed');",
    `INSERT INTO store_calendar_exceptions VALUES ('${LEGACY_TODAY_EXCEPTION_ID}', 'org-eye-seed');`,
