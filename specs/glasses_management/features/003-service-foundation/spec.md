...
**概要**: EYE予約を `services/example_service` の雛形から 0 で作り直す。この feature は
...
- AC-FOUND-01: Given `/s/:storeSlug` で置き場所を選んで暗証番号を通している, When 業務画面（`/`）を開く, Then 上のバーに店名と営業状態が出る。器は `header` と「画面の切り替え」の `nav` と `main` を 1 つずつ持ち、本文の先頭にいま開いている画面の名前が見出しとして置かれている。業務を始める入口そのもの（置き場所・暗証番号）はこの feature には無く、`013-terminals-and-audit` が持つ。（**2026-09-05 に改訂**。従来は「dev トークンで資格情報を受け取っている」を前提にしていたが、その dev グラント（`POST /api/auth/token`）は credential を検査せず本番では有効にできない経路であり、撤去した。前提を実運用の入口に置き換えただけで、この AC が確かめる器の形は変えていない。）
...
- AC-FOUND-03: Given 会社の管理者が業務画面に入っている, When その会社に店舗が 1 つも見つからない, Then **業務画面へは入れず**、最初のお店を登録する面を立てる。実在しない屋号も、どの店舗のものでもない営業状態も、押しても何も起きない行き先の柱も出さない。（**2026-09-05 に再改訂**。2026-09-03 版は入口で止めて「このコードのお店が見つかりませんでした。」と出していたが、店舗 0 件は打ち間違いと**まだ 1 店舗も登録していない新しい会社**の両方でありうる。入口で止めると後者は永久に入れないため、通したうえで登録の面を立てる — 打ち間違いの人も、その面が会社のコードを名乗るので自分が別の会社に入ったと分かる。詳細は `014-store-provisioning`。組織が admin からまだ届いていない 503 `not_synced` はこれとは別で、従来どおり通して AC-FOUND-07 の面が説明する。**2026-09-05 に再々改訂**: 入口が「お店のコード」から `/s/:storeSlug` の置き場所選択へ変わったので、Given から入力の手順を外した —— 店舗 0 件の会社に入ったときの振る舞いを言う AC であり、どうやって入ったかはこの AC の関心ではない。）
...
**スコープ外**: 端末の使い分けと暗証番号（`013-terminals-and-audit`）、店舗の登録（`004-store-settings`）、
予約・顧客・録音・分析・Web予約（それぞれの feature）。
**業務を始める入口そのもの**は `013-terminals-and-audit` が持つ。
2026-09-05 に dev トークングラント（`POST /api/auth/token`）と、それを呼ぶ暫定の「お店のコード」の画面を
**撤去した**。いまの入口は `/s/:storeSlug` →（置き場所を選ぶ）→ 暗証番号 → HOME である
...
- `packages/ui/src/theme.css` — 承認済みモック `docs/frontend/mockups/eye` のトークンへ全面的に書き直す
...
- [x] T-005: デザイントークンを `eye` モックの値へ書き直す。
