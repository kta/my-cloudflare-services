...
- AC-FOUND-03: Given 業務開始の画面でお店のコードを入れた, When そのコードで店舗が 1 つも見つからない, Then **業務の器を立てない**（左サイドバー「画面の切り替え」も、屋号も、営業状態も出さない）。かわりに「最初のお店を登録します」の面を立て、いまいる場所は「はじめの設定」と読める。（**2026-09-05 に再改訂**。2026-09-03 版は「入口に留めて『このコードのお店が見つかりませんでした。』と出す」だったが、それでは**まだ 1 店舗も登録していない新しい会社が永久に入れない**。未知の組織とまだ店舗が無い組織はサーバから見分けられない（どちらも `200 []`）ので、014-store-provisioning / AC-PROV-01 の「通したうえで最初のお店を登録させる」を採る。このACが守るのは「実在しない屋号と、どの店舗のものでもない固定の営業時間を出さない」ことであり、そこは面が変わっても守られている。さらに以前の版は「業務画面に入れてしまい『お店がまだ登録されていません。』と事実だけを出す」だった。組織が admin からまだ届いていない 503 `not_synced` はこれとは別で、従来どおり通して AC-FOUND-07 の面が説明する。）
